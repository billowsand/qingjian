...
只取 tag 形如 [平台-]v<版本> 的非草稿发布（平台前缀可选：windows- / linux-，各平台壳版本号独立），安装包按文件名识别平台与架构，其他附件（SHA256SUMS、build-info.json、releases.json 本身）不列。
...
TAG = re.compile(r"^(?:(?:windows|linux)-)?v(\d+\.\d+\.\d+(?:-[0-9A-Za-z.]+)?)$")
