...
pub(in crate::panel) fn repo_resource(rel: &str) -> Option<PathBuf> {
    qingjian_platform::resources::bundled_resource(rel)
}

pub(in crate::panel) fn open_in_editor(path: &Path) {
...
/// 资源管理器打开目录或网址。
pub(in crate::panel) fn open_with_explorer(target: &str) {
...
pub(in crate::panel) fn log_dir() -> Option<PathBuf> {
...
pub(in crate::panel) fn export_logs() {
