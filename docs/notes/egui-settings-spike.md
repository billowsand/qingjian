...
| 全部代码 | 1817 行 | 2094 行（含字体 / 配色 / GPU 后端这 200 行框架白送的东西） |
| 窗口大小 | 980×700 | **700×560** |
...
## 第三轮：按设计稿收拾版式（2026-09-18）

第二轮的界面「像 egui 画的」，不像设计过的产品。这一轮按四条原则重排：

- **一页一张列表卡**：页内不再有「输入方案 / 按键 / 标点」这类小节标题——分节已经在左侧导航里了，
  右边直接列选项，行与行之间只有一条从文字列起的淡分隔线（`widgets/list.rs`）。通用页九项一张卡，一屏看全。
- **图标与文字真正对齐**：图标是 Segoe Fluent 的私用区字形，`ascent` / `descent` 与汉字不同，
  靠布局居中一定偏上。改成由 painter 按 `Align2::CENTER_CENTER` 画在行的正中，再下移 1 px 补视觉重心差；
  导航项也是同一套做法（图标与文字分别 painter 画，`widget_info` 报的名字里也就没有那个私用区码点了）。
- **窗口收窄**：980×700 → 700×560（导航 168 + 控件列 184）。一行 38 px，比第二轮还紧。
- **文字能删就删**：关于页从「品牌 + 四列统计表 + 许可证 + 隐私 + 反馈」五大段压成
  「品牌 + 三行统计 + 四个按钮 + 一行许可证 + 折叠的数据来源」；词库页去掉每行都一样的图标列；
  「候选窗口」页原来顶着两张候选窗 + 状态条预览图，也删了——候选窗长什么样打字时就看见了，设置页只给选项。
- **控件形式统一**：每行右边只有一个下拉框或一个开关。「每页候选数」原来是拖数字的 `DragValue`，
  改成 1–9 的下拉，跟其他行一个样子。

顺带修的：悬停提示原先挂在整行的 `Response` 上**不出来**——行内的控件会挡住 hover 判定，
改挂在标签与 ⓘ 这两个 widget 上才稳。控件也从 egui 缺省的灰块改成「白底 + 1 px 描边 + 5 圆角」，与 Fluent 一致。
