...
title: 从第一次输入开始
order: 0
description: 字在输入法使用文档（Windows）：安装、按键、输入与纠错、云联想、词库、数据位置与卸载。
---

字在是一个用 Rust 写的拼音输入法：整句输入、简拼、拼写纠错、模糊音、双拼、英文模式，全部在本机完成。
...
- 字在在本机保存了哪些文件：[数据与日志](help/data-and-logs.md)。

## 三条原则

- **输入优先。** 任何增加延迟、弹窗与干扰的设计都视为错误。
- **全部在本机。** 拼音转换、词库、学习、排序均不联网。
- **可选的才联网。** 云联想缺省关闭；打开后数据直接发送给用户自行填写的服务商，不经过字在。
