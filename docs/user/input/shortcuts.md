...
`v` 与 `u` 两个前缀键可在「设置 → 快捷键」修改，只能是 v、u、i 三个字母（它们不作为任何拼音的开头）。
`u` 之后接拼音为 [问字](../cloud/question.md)，需打开云联想；`?` 缺省只是问号，可在「快捷键」页勾选让它也进入问字。
...
- 整段输入为英文词时直接给出英文候选。拼音不成立时（`hello` 只能切成 `he l l o`）英文排第一，拼音成立时（`china`）英文排第二。
  勾选「设置 → 通用」的「输入拼音时中文候选排在英文词前面」后英文固定排第二（`hello` 先 荷兰咯 再 hello，`key` 先 可以 再 key），
...
前几个候选中有对应 emoji 的词，emoji 紧随该词之后，右侧标注对应的词：`ku` → 哭 😢 😭，`smile` → smile 🙂 😊。
